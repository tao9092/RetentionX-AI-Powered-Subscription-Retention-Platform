...
const props = defineProps<{ customers: Customer[]; actions: RetentionAction[] }>()
...
const coveredHighRisk = computed(() => highRiskCustomers.value.filter((customer) => {
  return props.actions.some(action => action.customerId === customer.id && action.status !== 'Completed')
}).length)
...
const unprotectedRevenue = computed(() => highRiskCustomers.value
  .filter((customer) => !props.actions.some(action => action.customerId === customer.id && action.status !== 'Completed'))
...
<style scoped>
.insights-page{display:grid;gap:17px;width:min(100%,1540px);margin:0 auto;padding:24px 26px 42px}.page-intro,.panel-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:20px}.eyebrow{display:block;color:var(--rx-primary);font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.14em}h2{margin:8px 0 7px;color:var(--rx-ink);font-size:clamp(25px,3vw,34px);letter-spacing:-.05em}.page-intro p,.panel-heading p{max-width:680px;margin:0;color:var(--rx-muted);font-size:12px;line-height:1.55}.report-button{display:flex;align-items:center;gap:8px;min-height:38px;padding:0 12px;border:1px solid var(--rx-border-strong);border-radius:11px;color:#555c65;background:#fff;font-size:12px;font-weight:850;cursor:pointer}.insight-metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.insight-metrics article{display:flex;align-items:center;gap:11px;min-height:88px;padding:15px;border:1px solid var(--rx-border);border-radius:17px;background:#fff;box-shadow:var(--rx-shadow-sm)}.metric-icon{display:grid;place-items:center;width:39px;height:39px;border-radius:12px;font-size:12px;font-weight:900}.coverage{color:var(--rx-success);background:var(--rx-success-soft)}.exposed{color:var(--rx-danger);background:var(--rx-danger-soft)}.renewal{color:var(--rx-warning);background:var(--rx-warning-soft)}.utilisation{color:var(--rx-blue);background:var(--rx-blue-soft)}.insight-metrics small,.insight-metrics strong,.insight-metrics p{display:block}.insight-metrics small{color:#989da6;font-size:12px;font-weight:850;text-transform:uppercase}.insight-metrics strong{margin-top:5px;color:#30343a;font-size:15px}.insight-metrics p{margin:3px 0 0;color:#9ca1aa;font-size:12px}.panel{border:1px solid var(--rx-border);border-radius:19px;background:#fff;box-shadow:var(--rx-shadow-sm)}.matrix-panel,.hotspot-panel,.driver-panel,.renewal-panel{padding:18px}.panel-heading{align-items:flex-start}.panel-heading h3{margin:5px 0 4px;color:#30343a;font-size:13px}.text-button{border:0;color:var(--rx-primary);background:transparent;font-size:12px;font-weight:850;cursor:pointer}.small-label{color:#9ba0a8;font-size:12px;font-weight:850;text-transform:uppercase}.top-axis{display:flex;justify-content:space-around;margin:17px 0 7px 70px;color:#9ba0a8;font-size:12px;text-transform:uppercase;letter-spacing:.07em}.matrix-layout{display:grid;grid-template-columns:58px 1fr;gap:10px}.vertical-axis{display:flex;align-items:center;justify-content:center;writing-mode:vertical-rl;transform:rotate(180deg);color:#9ba0a8;font-size:12px;text-transform:uppercase;letter-spacing:.07em}.vertical-axis i{width:1px;height:38px;margin:8px 0;background:#dfe1e4}.matrix-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.segment{min-height:174px;padding:14px;border:1px solid;border-radius:14px}.segment.danger{border-color:#ffdbe1;background:#fff8f9}.segment.warning{border-color:#f5e1b6;background:#fffbf4}.segment.growth{border-color:#e2e0ff;background:#faf9ff}.segment.healthy{border-color:#c9f1eb;background:#f3fbfa}.segment-heading{display:flex;justify-content:space-between;gap:12px}.segment-heading span{color:#999fa8;font-size:12px;text-transform:uppercase}.segment-heading h4{margin:4px 0 0;color:#383d44;font-size:11px}.segment-heading>strong{display:grid;place-items:center;width:31px;height:31px;border-radius:10px;color:#171717;background:rgba(102,86,232,.1);font-size:11px}.segment>p{margin:9px 0;color:#858c95;font-size:12px;line-height:1.45}.segment-value{padding-top:9px;border-top:1px solid rgba(120,126,136,.13)}.segment-value span,.segment-value strong{display:block}.segment-value span{color:#999fa8;font-size:12px}.segment-value strong{margin-top:4px;color:#3d4249;font-size:12px}.account-chips{display:flex;align-items:center;gap:4px;margin-top:11px}.account-chips button,.more-chip,.empty-chip{display:grid;place-items:center;height:29px;border:0;border-radius:9px}.account-chips button{width:29px;color:#171717;background:var(--rx-primary-soft);font-size:12px;font-weight:900;cursor:pointer}.account-chips button small{display:none}.more-chip{width:29px;color:#858c95;background:#eff0f2;font-size:12px}.empty-chip{width:auto;padding:0 9px;color:#9ba0a8;background:#f0f1f3;font-size:12px}.analytics-grid{display:grid;grid-template-columns:1.08fr .92fr;gap:14px}.hotspot-list,.driver-list{display:grid;gap:7px;margin-top:13px}.hotspot-row{display:grid;grid-template-columns:22px minmax(120px,1fr) 90px 40px 72px;align-items:center;gap:8px;padding:9px 0;border-bottom:1px solid #eff0f2}.rank{color:var(--rx-primary);font-size:12px;font-weight:900}.hotspot-copy strong,.hotspot-copy small{display:block}.hotspot-copy strong{color:#41474f;font-size:12px}.hotspot-copy small{margin-top:3px;color:#9aa0a8;font-size:12px}.risk-meter,.driver-bar{height:5px;overflow:hidden;border-radius:99px;background:#eceef0}.risk-meter i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--rx-primary),var(--rx-danger))}.risk-number{color:var(--rx-danger);font-size:12px}.risk-revenue{color:#7c838c;font-size:12px;text-align:right}.driver-row{display:grid;grid-template-columns:minmax(150px,1fr) .85fr;gap:12px;align-items:center}.driver-row>div:first-child{display:flex;justify-content:space-between;gap:8px}.driver-row strong{color:#454b53;font-size:12px}.driver-row span{color:#999fa8;font-size:12px}.driver-bar i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--rx-primary),#2d7ff9)}.renewal-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:13px}.renewal-grid article{padding:13px;border:1px solid var(--rx-border);border-radius:13px}.renewal-grid article.critical{border-color:#ffdbe1;background:#fff8f9}.renewal-grid article.attention{border-color:#f5e1b6;background:#fffbf4}.renewal-grid article.planned{border-color:#e2e0ff;background:#faf9ff}.renewal-grid article.stable{border-color:#c9f1eb;background:#f3fbfa}.renewal-grid article>span{color:#969ca5;font-size:12px;text-transform:uppercase}.renewal-grid article>strong{display:block;margin-top:7px;color:#3b4047;font-size:12px}.renewal-grid article>p{margin:4px 0 0;color:#858c95;font-size:12px}.mini-accounts{display:flex;gap:4px;margin-top:10px}.mini-accounts button,.mini-accounts i{display:grid;place-items:center;width:27px;height:27px;border:0;border-radius:8px;color:#171717;background:var(--rx-primary-soft);font-size:12px;font-weight:900;font-style:normal;cursor:pointer}.mini-accounts i{color:#858c95;background:#eff0f2}.disclaimer{margin:0 3px;color:#9ba0a9;font-size:12px;line-height:1.5}@media(max-width:1120px){.insight-metrics{grid-template-columns:repeat(2,1fr)}.analytics-grid{grid-template-columns:1fr}.renewal-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:720px){.insights-page{padding:17px 14px 30px}.page-intro{align-items:flex-start;flex-direction:column}.matrix-layout{grid-template-columns:1fr}.vertical-axis,.top-axis{display:none}.matrix-grid{grid-template-columns:1fr}.hotspot-row{grid-template-columns:22px 1fr 40px}.risk-meter,.risk-revenue{display:none}.renewal-grid{grid-template-columns:1fr}}@media(max-width:480px){.insight-metrics{grid-template-columns:1fr}}@media print{.report-button,.text-button{display:none}.insights-page{padding:0}.panel,.insight-metrics article{box-shadow:none;break-inside:avoid}}
</style>
