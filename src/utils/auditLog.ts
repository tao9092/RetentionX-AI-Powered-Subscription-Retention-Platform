...
export function auditEvent(type:AuditEvent['type'],detail:string,actor='Local demo user',timestamp=new Date().toISOString()):AuditEvent{return{id:`audit-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,type,detail,actor,timestamp}}
